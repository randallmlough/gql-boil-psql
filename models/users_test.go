...
func testUserToManyOwnerPets(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	if err = randomize.Struct(seed, &b, petDBTypes, false, petColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}
	if err = randomize.Struct(seed, &c, petDBTypes, false, petColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}
	...
	check, err := a.OwnerPets().All(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	slice := UserSlice{&a}
	if err = a.L.LoadOwnerPets(ctx, tx, false, (*[]*User)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.OwnerPets); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
	}

	a.R.OwnerPets = nil
	if err = a.L.LoadOwnerPets(ctx, tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.OwnerPets); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testUserToManyAddOpOwnerPets(t *testing.T) {
	var err error
	...
	foreigners := []*Pet{&b, &c, &d, &e}
	for _, x := range foreigners {
		if err = randomize.Struct(seed, x, petDBTypes, false, strmangle.SetComplement(petPrimaryKeyColumns, petColumnsWithoutDefault)...); err != nil {
			t.Fatal(err)
		}
	}
	...
	foreignersSplitByInsertion := [][]*Pet{
		{&b, &c},
		{&d, &e},
	}

	for i, x := range foreignersSplitByInsertion {
		err = a.AddOwnerPets(ctx, tx, i != 0, x...)
		if err != nil {
			t.Fatal(err)
		}
		...
		if a.R.OwnerPets[i*2] != first {
			t.Error("relationship struct slice not set to correct value")
		}
		if a.R.OwnerPets[i*2+1] != second {
			t.Error("relationship struct slice not set to correct value")
		}

		count, err := a.OwnerPets().Count(ctx, tx)
		if err != nil {
			t.Fatal(err)
		}
...
func testUserToManySetOpOwnerPets(t *testing.T) {
	var err error
	...
	err = a.SetOwnerPets(ctx, tx, false, &b, &c)
	if err != nil {
		t.Fatal(err)
	}
	...
	err = a.SetOwnerPets(ctx, tx, true, &d, &e)
	if err != nil {
		t.Fatal(err)
	}
	...
	if a.R.OwnerPets[0] != &d {
		t.Error("relationship struct slice not set to correct value")
	}
	if a.R.OwnerPets[1] != &e {
		t.Error("relationship struct slice not set to correct value")
	}
}

func testUserToManyRemoveOpOwnerPets(t *testing.T) {
	var err error
	...
	err = a.AddOwnerPets(ctx, tx, true, foreigners...)
	if err != nil {
		t.Fatal(err)
	}
	...
	err = a.RemoveOwnerPets(ctx, tx, foreigners[:2]...)
	if err != nil {
		t.Fatal(err)
	}
	...
	if len(a.R.OwnerPets) != 2 {
		t.Error("should have preserved two relationships")
	}

	// Removal doesn't do a stable deletion for performance so we have to flip the order
	if a.R.OwnerPets[1] != &d {
		t.Error("relationship to d should have been preserved")
	}
	if a.R.OwnerPets[0] != &e {
...
var (
	userDBTypes = map[string]string{`ID`: `integer`, `CreatedAt`: `timestamp with time zone`, `UpdatedAt`: `timestamp with time zone`, `DeletedAt`: `timestamp with time zone`, `Email`: `character varying`, `Name`: `character varying`, `Password`: `text`}
	_           = bytes.MinRead
)
